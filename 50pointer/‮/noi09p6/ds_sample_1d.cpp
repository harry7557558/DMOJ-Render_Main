...
void initTree(node* R, const vector<capsule> &caps) {
	...
	for (int i = 0; i < caps.size(); i++) {
		...
		if (x1 - x0 > 0.5*dx) R->vcap.push_back(caps[i]);
			...
			// in the test cases, the lengths of cap1 and cap2 are non-zero
		}
	}
...
#if USE_TREE
	vector<capsule> vcaps(Cap, Cap + M);
	sort(vcaps.begin(), vcaps.end(), [](capsule a, capsule b) { return a.a.x + a.b.x < b.a.x + b.b.x; });
	initTree(&R, vcaps);
#endif
